...
在 **网络与安全 → Web 访问 / 网络** 中，打开「**允许外部访问（局域网/公网）**」开关即可让局域网内其他设备访问。

::: warning 安全提醒
开启外部访问后，请务必设置 Web 访问密码，否则同一网络内的任何人都能访问你的 Agent。
:::
...
为 Web 界面设置访问密码，适用于局域网或公网暴露场景：

1. 在 [高级配置](/web/#/config/advanced) 中找到「Web 访问密码」
2. 输入密码并保存
3. 之后访问 Web 界面时需输入密码

密码以 bcrypt 哈希存储在本地，不会明文保存。
